...
Fila *newFila()
{
    ...
    new->personagem = (Personagem *)malloc(TAM * sizeof(Personagem));
    new->primeiro = new->ultimo = 0;
    return new;
}
...
Personagem Remover(Fila *fila)
{
    ...
    removido = fila->personagem[fila->primeiro];
    fila->primeiro = (fila->primeiro + 1) % TAM;
    return removido;
}

/**
 * funcao para inserir no fim da fila
 */
void Inserir(Personagem p, Fila *fila)
{
    Personagem removido;
    ...
    fila->personagem[fila->ultimo] = p;
    fila->ultimo = (fila->ultimo+1)%TAM;
}

/**
 * funcao para mostra os elementos da fila separados por espacos.
 */
void Mostrar(Fila *fila)
{
    int i;
    printf("[ ");
    for (i = fila->primeiro; i != fila->ultimo; i = ((i + 1) % TAM))
    {
        printf("%s\n", fila->personagem[i].id);
    }
    printf("] \n");
}
...
{
    return fila->personagem[pos].id;
}

/**
 * funcao para calcular media do ano de nascimento
*/
int anoMedio(Fila *fila)
{
    int i;
    int resp = 0;
    int count = 0;
    for (i = fila->primeiro; i != fila->ultimo; i = ((i + 1) % TAM))
    {
        resp += atoi(fila->personagem[i].yearOfBirth);
        count++;
    }
    if (count == 0)
    {
...
int main()
{
    ...
    //--------------------------------------------------------------------//
    //-----------TP03Q04-Fila-Circular-com-Alocação-Sequencial------------//

    /*
    Crie uma classe Fila Circular de Personagem.
    ...
    Esse número corresponde à média arredondada do yearOfBirth dos registros contidos na fila após cada inserção.

    Além disso, para cada registro removido da fila, a saída padrão também apresenta a palavra (R) e alguns atributos desse registro.
    ...
    */
    ...
    while (strcmp(input, "FIM") != 0)
    {
        ...
        {
            Inserir(getElementById(input), fila);
            media = anoMedio(fila);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (strcasecmp(comando, "I") == 0)
        {
            char id[50];
            scanf("%s", id);
            Inserir(getElementById(id), fila);
            media = anoMedio(fila);
            printf(">> Year Birthday Average: %d\n", media);
        }
    ...
    int count = 0;
    printf("[ Head ]\n");
    for (int i = fila->primeiro; i != fila->ultimo; i = ((i + 1) % TAM))
    {
        printf("[%d ## ", count);
        imprimir(getElementById(getID(fila, i)));
        count++;
    }
    printf("[ Tail ]\n");
