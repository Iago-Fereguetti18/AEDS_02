...
void empilhar(Pilha *pilha, Personagem p)
{
    Celula *tmp = newCell(p);
    tmp->prox = pilha->topo;
    pilha->topo = tmp;
    tmp = NULL;
}

Personagem desempilhar(Pilha *pilha)
{
    if (pilha->topo == NULL)
    {
    ...
    Personagem elemento = pilha->topo->personagem;
...
int main()
{
    ...
    while (strcmp(input, "FIM") != 0)
    {
        ...
        {
            empilhar(pilha, getElementById(input));
        }
        scanf("%s", input);
    }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (strcasecmp(comando, "I") == 0)
        {
            char id[50];
            scanf("%s", id);
            empilhar(pilha, getElementById(id));
        }
