...
int anoMedio(Fila *fila)
{
    ...
    do
    {
        resp = resp + atoi(i->personagem.yearOfBirth);
        count++;
        i = i->prox;
...
int main()
{
    ...
    //--------------------------------------------------------------------//
    //-----------=----TP03Q07-Fila-com-Alocação-Flexível------------------//
    ...
    {
        printf("[%d ## ", count);
        imprimir(getElementById(getID(fila, pos)));
        pos++;
        i = i->prox;
        count++;
    }while (i != fila->primeiro);
