...
No *newNo(Personagem p, No *esq, No *dir, int nivel)
{
    No *new = (No *)malloc(sizeof(No));
    new->elemento = p;
    new->esq = esq;
...
No *inserir(Personagem p, No *i)
{
    if (i == NULL)
    {
        i = newNoFirt(p);
    }
    else if (strcmp(p.name, i->elemento.name) < 0)
    {
...
void inserirFirt(Personagem p)
{
    raiz = inserir(p, raiz);
}

void pesquisar(Personagem p, No *i)
{
    if (i == NULL)
    {
        printf("NAO\n");
...
void pesquisarFirt(Personagem p)
{
    printf("raiz ");
    pesquisar(p, raiz);
}

Personagem getElementByName(char *name)
{
    Cell *i;
    for (i = primeiro->prox; i != NULL; i = i->prox)
    {
...
int main()
{
    ...
    while (strcmp(input, "FIM") != 0)
    {
        ...
        {
            inserirFirt(getElementById(input));
        }
        scanf("%s", input);
    }
    ...
    while (strcmp(input, "FIM") != 0)
    {
        ...
        {
            printf("%s => ", input);
            pesquisarFirt(getElementByName(input));
        }
        scanf(" %[^\r\n]%*c", input);
    }
