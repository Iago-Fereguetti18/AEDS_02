...
void insert(HashTable *hashtable, Personagem p) {
    ...
    if (hashtable->table[index] == NULL) {
        ...
        while (current->next != NULL && strcmp(current->key.name, p.name) != 0) {
            current = current->next;
        }
        if (strcmp(current->key.name, p.name) == 0) {
            current->key = p;  // Atualiza o valor
            free(new_node);
        } else {
...
void search(HashTable *hashtable, char *key) {
    ...
    Node *current = hashtable->table[index];
    while (current != NULL) {
        if (strcmp(current->key.name, key) == 0) {
            printf("%s (pos: %d) SIM\n", current->key.name, index);
            return;
        }
        current = current->next;
    }
...
int main()
{
    ...
    while (strcmp(input, "FIM") != 0)
    {
        ...
        {
            insert(tabela, getElementById(input));
        }
        scanf("%s", input);
    }
