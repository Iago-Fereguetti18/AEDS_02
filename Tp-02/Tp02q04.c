...
void registrarLog() {
    FILE *arquivo_log;
    arquivo_log = fopen("815991_binaria.txt", "a"); // "a" para adicionar ao arquivo existente
    if (arquivo_log == NULL) {
...
// ----------------------------------------------------------------------------------------//
// -------------------------------Questão 04_Pesquisa Binária------------------------------//

bool pesquisaBinaria(String *idarr, int n, char procura[50], int *comparacoes) {
    int dir = n - 1, esq = 0;
    //bool resp = false;
    while (esq <= dir) {
        int meio = (esq + dir) / 2;
...
void quicksort(String *idarr, int n, int esq, int dir, int *comparacoes, int *movimentacoes) {
    int i = esq, j = dir;
    int pivo = (dir + esq) / 2;
    char tmp[50];
    Personagem personagem1 = getElementById(idarr[pivo]);
    while (i <= j) {
        while (strcmp(getElementById(idarr[i]).name, personagem1.name) < 0) {
            i++;
            (*comparacoes)++;
        }
        while (strcmp(getElementById(idarr[j]).name, personagem1.name) > 0) {
            j--;
            (*comparacoes)++;
        }
        if (i <= j) {
            ...
            i++;
            j--;
            (*movimentacoes) += 3; // 3 movimentações: 2 copias e 1 swap
        }
    }
    if (esq < j) {
        quicksort(idarr, n, esq, j, comparacoes, movimentacoes);
    }
    if (i < dir) {
        quicksort(idarr, n, i, dir, comparacoes, movimentacoes);
    }
}
...
void TP02Q04() {
    ...
    int comparacoes = 0;
    int movimentacoes = 0;
    quicksort(idarr, x, 0, x - 1, &comparacoes, &movimentacoes);

    bool achou = false;
    scanf(" %[^\r\n]%*c", input);
    while (strcmp(input, "FIM") != 0) {
        achou = pesquisaBinaria(idarr, x, input, &comparacoes);
        if (achou) {
            printf("SIM\n");
        } else {
            printf("NAO\n");
...
int main()
{
    ...
    {
        reset(&personagens);      // resetar as variaveis
        ler(&personagens, line); // ler do arquivo e armazenar as variaveis
        addend(personagens);      // adicionar na lista encadeada
        elenco++;
    }
    /*
    Cell *atual = primeiro->prox; // Começa do primeiro elemento real da lista
    while (atual != NULL) {
        imprimir(atual->personagem); // Imprime as informações do personagem atual
