...
void registrarLog() {
    FILE *arquivo_log;
    arquivo_log = fopen("815991_shellsort.txt", "a"); // "a" para adicionar ao arquivo existente
    if (arquivo_log == NULL) {
...
// ----------------------------------------------------------------------------------------//
// -----------------------------------Questão_08_Shellsort---------------------------------//

void insercaoPorCor(String *idarr, int n, int cor, int h) {
    char tmp[500];
    Personagem p2;
    for (int i = (h + cor); i < n; i += h) {
        p2 = getElementById(idarr[i]);
        strcpy(tmp, idarr[i]);
        int j = i - h;
        while ((j >= 0) && ((strcmp(getElementById(idarr[j]).eyeColour, p2.eyeColour) > 0) ||
                            ((strcmp(getElementById(idarr[j]).eyeColour, p2.eyeColour) == 0) &&
                             (strcmp(getElementById(idarr[j]).name, p2.name) > 0)))) {
            strcpy(idarr[j + h], idarr[j]);
            atualizarMovimentacoes(1); // Atualizar movimentações
            atualizarComparacoes(2);   // Atualizar comparações
            j -= h;
        }
        strcpy(idarr[j + h], tmp);
        atualizarMovimentacoes(1); // Atualizar movimentações
    }
}

void shellsort(String *idarr, int n) {
    int h = 1;
